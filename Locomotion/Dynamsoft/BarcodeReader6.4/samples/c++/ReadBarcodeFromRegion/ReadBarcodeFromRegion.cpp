...
int DecodeFile(CBarcodeReader& reader,const char* pszImageFile,int mode,int iLeft,int iTop,int iRight,int iBottom,float& fCostTime)
{
    ...
    snprintf(tempDefaultTemplateJson,1024,"{\"Version\": \"2.0\",\"ImageParameter\": {\"Name\": \"%s\",\"BarcodeFormatIds\": [\"All\"],\"RegionPredetectionMode\": \"Enable\"}}",pDefaultTemplate);

	// Mode 2: Specify the barcode area manually.
    char tempTemplateJsonWithRegion[1024]={0};
    snprintf(tempTemplateJsonWithRegion,1024,"{\"Version\": \"2.0\",\"ImageParameter\": {\"Name\": \"%s\",\"BarcodeFormatIds\": [\"All\"],\"RegionPredetectionMode\": \"Disable\",\"RegionDefinitionNameArray\": [\"Region\"]},\"RegionDefinitionArray\": [{\"Name\": \"Region\",\"MeasuredByPercentage\": true,\"Left\":%d,\"Top\":%d,\"Right\":%d,\"Bottom\":%d}]}",pDefaultTemplateWithRegion,iLeft,iTop,iRight,iBottom);


    char error[1024] ={0};
    if(mode == 0)
    {
    	iResult = reader.AppendTplStringToRuntimeSettings(tempDefaultTemplateJson,ECM_Overwrite,error,1024);

    }else
    {
        iResult = reader.AppendTplStringToRuntimeSettings(tempTemplateJsonWithRegion,ECM_Overwrite,error,1024);
    }
...
int main(int argc, const char* argv[])
{
	...
	while(1)
	{
		...
		reader.InitLicense("t0068NQAAACA+Sw8UTdw2lUgUuQyCzB90v4yFGXXZCbwzAtnU2nm3QcVFODHhnM+a22gZSBSdQ3Yz9G1mgtCkJ1NTeFAW+X8=");
